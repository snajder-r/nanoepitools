...
def plot_met_profile(matrix: np.array, samples: np.ndarray = None,
                     sample_order: List[str] = None, sample_colors: Dict = None,
                     site_genomic_pos=None, site_genomic_pos_end=None,
                     marker_height=0.75, segment: np.array = None,
    ...
    for s in sample_order:
        ...
        if site_genomic_pos_end is None:
            plt.scatter(x, y, c=color, marker='|',
                        s=15 * (0.25 + marker_height))
        else:
            # if the end location for each marker is given, we need to plot
            # rectangles
            patches = [
                Rectangle((x[i], y[i]), x_end[i] - x[i] + 1, marker_height) for
                i in range(len(x))]
    ...
    if highlights is not None:
        plot_vertical_highlights(highlights, highlight_color=highlight_color,
                                 site_genomic_pos=site_genomic_pos)


def plot_vertical_highlights(highlights: List[Tuple], highlight_color='b',
